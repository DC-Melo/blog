TARGET:DAWN → SUNRISE → MORNING → NOON → AFTERNOON → SUNSET → EVENING → NIGHT → @20200525 → ✓✓? ⇒ 
●:vimium to control focus ⇒ cvim not work again ⇒ FREZEN
●:make monkey page and audio test ⇒ ✓
●:make gdb note ⇒ ✓
td-dc:@20200526 → ?✓? → @20200527 → ??? → @20200529 → ??? → @20200601 → ??? → @20200609 → ??? ⇒ @20200625 → ??? ⇒ 
1.read books about c++ 2.read code about leetcode. 3.
mutt -s 12518-22Mon20-20200525 melo.dachor@gmail.com < $dbg/12518-22Mon20-20200525

SUNRISE:
Mon 25 May 2020 07:00:30 AM CST
    A.get up    

Mon 25 May 2020 10:24:01 AM CST
MORNING:
    GDB not work
    ...
    Navigate to the directory with the code from above.

    use:vimium + cvim
Mon 25 May 2020 10:29:43 AM CST
NOON:
AFTERNOON:
    A.CGDB not installed.
    install CGDB:
    Dependencies
    ...
    STL可分为容器(containers)、迭代器(iterators)、空间配置器(allocator)、配接器(adapters)、算法(algorithms)、仿函数(functors)
...
(gdb) handle signo ac-dcs      set debugger ac-dcs for signal
